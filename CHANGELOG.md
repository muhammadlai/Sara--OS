...
All notable changes to AITZAZ AI 2070 (formerly the B2B SDR Agent Template) are documented here.
...
### Changed (branding only — no functional changes)
- Application/user-facing identity: B2B SDR Agent Template → **AITZAZ AI 2070**; package renamed to `aitzaz-ai-2070` (v4.0.0); skill renamed to `aitzaz-ai-2070` (SKILL.md)
- Workspace identity pinned: company/brand = AITZAZ AI 2070, owner = Aitzaz (USER.md, IDENTITY.md, AGENTS.md, product-kb/catalog.json)
- Upstream marketing removed from active docs: PulseAgent SaaS upsells, Product Hunt / Buy-Me-a-Coffee badges, third-party analytics ping in install.sh, managed-hosting menu path (upstream SaaS links remain only inside `whatsapp-old-account-onboarding/` docs, where they describe that external integration, and in historical records — CHANGELOG history, `.sync/`, `docs/internal/`)
- `install.sh` now clones this repository (muhammadlai/Sara--OS); banner, docs links and post-install guidance rebranded
- All 8 localized READMEs rebranded; clone URLs updated; managed-hosting promo blocks replaced with dashboard instructions
- SECURITY.md: vulnerability reports now go through GitHub Security Advisories on this repository
- .github/FUNDING.yml: third-party funding links removed
- Social preview (social-preview.html/.png) rebranded

### Added
- `dashboard/` — futuristic AI-OS control surface (zero-dependency static page). All panels render data generated from the real project files by `scripts/build-dashboard-data.mjs` (workspace layers, 8 skills, 10 pipeline stages, 14 heartbeat checks, product KB, validator results, git history). Non-existent capabilities are explicitly listed under Future Phases, never faked. Run: `npm run dashboard:data`, then open `dashboard/index.html`
- package.json scripts: `dashboard:data`, `dashboard`

### Preserved
- LICENSE (original copyright attribution untouched), all skills, agents, workspace layers, deploy scripts, tests (`npm test` — all 8 checks pass), CI workflow, examples, WhatsApp onboarding toolkit, docs, `.sync/` history
