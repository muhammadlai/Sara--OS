# AITZAZ AI 2070 — Modèle d'Agent SDR B2B
...
|---------|--------|------------|
| `b2b_trade` | 41 skills | Entreprises d'export B2B (par défaut) |
...
### Tableau de bord AITZAZ AI 2070

Le dépôt inclut un tableau de bord futuriste de système d'exploitation IA (`dashboard/`). Il affiche des données générées depuis les vrais fichiers du projet (couches du workspace, skills, KB produits, validation) — sans données fictives ; les capacités inexistantes sont marquées comme phases futures.

```bash
npm run dashboard   # régénère dashboard/data.json
