# AITZAZ AI 2070 — B2B SDR エージェントテンプレート
...
| プロフィール | スキル数 | 最適な用途 |
|------------|---------|----------|
| `b2b_trade` | 41 スキル | B2B 輸出企業（デフォルト） |
...
### AITZAZ AI 2070 ダッシュボード

未来的な AI-OS ダッシュボード（`dashboard/`）を同梱。実際のプロジェクトファイル（ワークスペース層・スキル・製品KB・検証結果）から生成したデータを表示し、モックなし。未実装の機能は将来フェーズとして明示。

```bash
npm run dashboard   # dashboard/data.json を再生成
