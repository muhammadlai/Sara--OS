# Sara--OS · B2B SDR Agent

> **AITZAZ AI** — Open-source, production-ready AI SDR template for OpenClaw deployments.
...
> 📦 **Add-on · 2026-05-22** — **[WhatsApp Legacy Account Onboarding Spec v0.5](./whatsapp-old-account-onboarding/docs/README.md)** *(Path D — Multi-Device fetch)*: Lowest-friction history acquisition — customer scans one QR, waits for Multi-Device sync, `bootstrap.sh` pulls .txt files straight from AITZAZ AI. No backup, no password, no USB. Backup-extraction (v0.4) still default for zero-downtime tenants. Three-layer architecture (MemOS + sales_playbook + conversation_history), A/B/C/D delivery decision tree, strict auto-onboard gate. [Release notes →](https://github.com/muhammadlai/Sara--OS/releases/tag/whatsapp-onboarding-v0.5)

An open-source, production-ready template by **AITZAZ AI** for building AI Sales Development Representatives (SDRs) that handle the **full sales pipeline** — from lead capture to deal closing — across WhatsApp, Telegram, and email.
...
### AITZAZ AI SDR Console (static)

This repository also ships a self-contained product console at [`dashboard/index.html`](./dashboard/index.html) — a polished, dependency-free overview of the SDR agent (leads, scoring, pipeline, campaigns, outreach, replies, memory, skills, system health). Open it in any browser, or serve it from the repo root:
...
No build step, no API keys, no external assets required.
...
Validation is one command: `npm test` (runs [`scripts/validate-template.sh`](./scripts/validate-template.sh)). A GitHub Actions workflow for CI is included at [`docs/ci-validate.yml`](./docs/ci-validate.yml) — restore it to `.github/workflows/validate.yml` to run the same checks on every push and pull request.
...
Don't want to self-host? **[AITZAZ AI](https://pulseagent.io/app)** offers fully managed B2B SDR agents with:
...
**Learn more:**
- [AITZAZ AI Blog](https://pulseagent.io/en/blog) — Tutorials, release notes, and case studies
...
<p align="center">
  Built with ❤️ by <a href="https://pulseagent.io/app">AITZAZ AI</a><br/>
