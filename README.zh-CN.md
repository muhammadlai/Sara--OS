# AITZAZ AI 2070 — B2B SDR Agent 模板
...
> 📦 **附加模块 · 2026-05-22** — **[WhatsApp 老号接入规范 v0.5](./whatsapp-old-account-onboarding/docs/README.zh-CN.md)** *（Path D — Multi-Device 同步）*：摩擦最小的历史获取路径——客户扫一次 QR，等 Multi-Device 同步，`bootstrap.sh` 直接从已连接账号拉 .txt。无备份、无密码、无 USB。v0.4 备份提取仍是零停机租户的默认。三层架构（MemOS + sales_playbook + conversation_history）、A/B/C/D 交付决策树、严格自动接入闸。
...
仓库内置未来感 AI 操作系统仪表盘（`dashboard/`），展示由真实项目文件（工作区层级、技能、产品知识库、校验结果）生成的数据——无模拟数据；尚未实现的能力明确标注为未来阶段。

```bash
npm run dashboard   # 重新生成 dashboard/data.json
