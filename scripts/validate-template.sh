...
# CI workflow lives at docs/ci-validate.yml (GitHub Apps without the
# `workflows` permission cannot push files under .github/workflows/).
# Restore it to .github/workflows/validate.yml when Actions permissions
# are available; the validator accepts either location.
ci_workflow=""
if [ -f "$ROOT/.github/workflows/validate.yml" ]; then
  ci_workflow=".github/workflows/validate.yml"
elif [ -f "$ROOT/docs/ci-validate.yml" ]; then
  ci_workflow="docs/ci-validate.yml"
else
  fail "Missing CI workflow (expected .github/workflows/validate.yml or docs/ci-validate.yml)"
fi
pass "CI workflow present ($ci_workflow)"
