...
case "${DELIVERY_PATH}" in
    ...
    B|C)
        ...
        ask_choice "Extraction method?" EXTRACT_METHOD \
            "Path D — fetch from AITZAZ AI (no USB, no password; needs Multi-Device sync done)" \
        ...
        if [[ "${EXTRACT_METHOD}" == Path\ D* ]]; then
            ...
            info "Fetching imported history from AITZAZ AI..."
...
# ---- Step 5b: optional push to AITZAZ AI -----------------------------------

if [[ "${DELIVERY_PATH}" != "A" ]]; then
    say ""
    ask_choice "Push to AITZAZ AI now?" PUSH_NOW \
    ...
    if [[ "${PUSH_NOW}" == Yes* ]]; then
        if [[ ! -f "${HOME}/.pa-config.json" && -z "${PA_ENDPOINT:-}" ]]; then
            ...
            ask "AITZAZ AI endpoint URL (e.g. https://pa.example.com)" PA_ENDPOINT
            ask "AITZAZ AI API token (Bearer)" PA_TOKEN
